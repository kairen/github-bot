...
func TestHandler(t *testing.T) {
	...
	mux.Handle("/webhooks", Handler([]Webhook{fakeHook}))

	s := httptest.NewServer(Handler([]Webhook{fakeHook}))
	defer s.Close()
...
func TestRun(t *testing.T) {
	...
	go Run([]Webhook{fakeHook}, "127.0.0.1:3006", "/webhooks")
	time.Sleep(5000)
...
func TestRunServer(t *testing.T) {
	...
	go RunServer(server, []Webhook{fakeHook}, "/webhooks")
	time.Sleep(5000)
...
`
	...
	go RunTLSServer(server, []Webhook{fakeHook}, "/webhooks")
	time.Sleep(5000)
