...
func Handler(hooks []Webhook) http.Handler {
	return &server{
		hooks: hooks,
	}
}
...
// Run runs a server
func Run(hooks []Webhook, addr string, path string) error {
	srv := &server{
		hooks:            hooks,
		path:             path,
...
func RunServer(s *http.Server, hooks []Webhook, path string) error {
...
func RunTLSServer(s *http.Server, hooks []Webhook, path string) error {
...
func (s *server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, hook := range s.hooks {
		event := r.Header.Get(getEventHeader(hook.Provider()))
		if len(event) != 0 {
...
func getEventHeader(p Provider) string {
	switch p {
	case GitHub:
